...
namespace DMAHR.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Login(LoginViewModel model, string returnUrl)
        {
            ...
            {
                Account result = _account.Query(u => u.Email == model.Email && u.Password == model.Password).FirstOrDefault();
                if (result != null)
                {
                    FormsAuthentication.SetAuthCookie(model.Email, model.RememberMe);
                    SetCookie("accCookie", result.ID, result.Username, result.Role, result.Email);
                    return RedirectToAction("Index", "Personal");
                }
                else
                {
                    ViewBag.Unauthorize = "Please enter valid email and password";
                }
                return View();
            }
            return View();
        ...
        public void SetCookie(string CookieName, int AccountId, string UserName, string Role, string Email)
        {
        ...
        public ActionResult LogOff()
        {
            ...
            Response.Cookies["accCookie"].Expires = DateTime.Now.AddYears(-30);
            Session.Clear();
            return RedirectToAction("Login", "Account");
        }
